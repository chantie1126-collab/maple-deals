...
function Pill({ children, tone = "light" }) {
  const classes =
    tone === "gradient"
      ? "bg-[linear-gradient(135deg,#d70000_0%,#ff7a00_100%)] text-white shadow-[0_4px_0_0_rgba(0,0,0,0.9)]"
      : tone === "dark"
  ...
  return (
    <span className={`inline-flex rounded-full border-2 border-black px-3 py-1 text-xs font-bold ${classes}`}>
      {children}
    </span>
  );
}

function PrimaryButton({ href, children, small = false }) {
  const className = small
    ? "inline-flex items-center gap-2 rounded-2xl border-2 border-black bg-[linear-gradient(135deg,#d70000_0%,#ff7a00_100%)] px-4 py-2 text-sm font-bold text-white shadow-[0_4px_0_0_rgba(0,0,0,0.9)] transition hover:translate-y-[1px]"
    : "inline-flex items-center gap-2 rounded-2xl border-2 border-black bg-[linear-gradient(135deg,#d70000_0%,#ff7a00_100%)] px-5 py-4 text-base font-bold text-white shadow-[0_6px_0_0_rgba(0,0,0,0.9)] transition hover:translate-y-[1px]";
...
function SecondaryButton({ children }) {
  return (
    <button
      type="button"
      className="inline-flex items-center gap-2 rounded-2xl border-2 border-black bg-white px-5 py-4 text-base font-bold text-neutral-900 shadow-[0_6px_0_0_rgba(0,0,0,0.9)] transition hover:translate-y-[1px]"
    >
      {children}
    </button>
  );
...
function DealCard({ deal }) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <div className="group h-full overflow-hidden rounded-[2rem] border-2 border-black bg-white shadow-[0_14px_0_0_rgba(0,0,0,0.9)] transition-transform duration-200 hover:-translate-y-1">
        <div className="relative overflow-hidden border-b-2 border-black">
          ...
          <div className="absolute left-4 top-4 flex flex-wrap gap-2">
            <Pill tone="gradient">{deal.discount}</Pill>
            <Pill>{deal.category}</Pill>
          </div>
        </div>

        <div className="flex h-[calc(100%-13rem)] flex-col p-5">
          <div className="mb-3 flex items-start justify-between gap-3">
            <h3 className="text-lg font-black leading-tight text-neutral-950">{deal.title}</h3>
          </div>
          ...
          <div className="mt-auto flex items-center justify-between gap-3">
            <span className="text-xs uppercase tracking-[0.2em] text-neutral-500">Maple Deals Pick</span>
            <PrimaryButton href={deal.link} small>
...
function FeaturedCard({ deal }) {
  return (
    <div className="rounded-[1.6rem] border-2 border-black bg-white p-4 shadow-[0_8px_0_0_rgba(0,0,0,0.9)]">
      <img
        src={deal.image}
        alt={deal.title}
      ...
      <div className="mb-2 flex items-center gap-2">
        <Pill tone="dark">Featured</Pill>
        <Pill tone="gold">{deal.discount}</Pill>
      </div>
            ...
            <div>
              ...
              <div className="mb-6 flex items-center gap-4">
                <div className="flex h-24 w-24 items-center justify-center rounded-[1.75rem] border-2 border-black bg-white p-2 shadow-[0_8px_0_0_rgba(0,0,0,0.9)]">
                  <img src={brand.logo} alt="Maple Deals logo" className="h-full w-full object-contain" />
                </div>
                <div>
                  <p className="text-sm font-black uppercase tracking-[0.32em] text-neutral-500">Maple Deals</p>
              ...
              <div className="mt-8 grid max-w-2xl gap-4 sm:grid-cols-3">
                {[
                  { label: "Deals Posted", value: publishedDeals.length ? String(publishedDeals.length) : "0" },
                  ...
                  >
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-neutral-500">{stat.label}</p>
            ...
            <div className="relative">
              ...
              <div className="rounded-[2rem] border-2 border-black bg-[linear-gradient(180deg,#111111_0%,#1f1f1f_100%)] p-5 pt-10 shadow-[0_14px_0_0_rgba(0,0,0,0.9)]">
                <div className="mb-4 flex items-center justify-between rounded-2xl border-2 border-black bg-white px-4 py-3">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-neutral-500">Featured This Week</p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12 md:px-10 lg:px-12">
        <div className="mb-8 grid gap-4 md:grid-cols-3">
          {[
            ...
            >
              <p className="text-xs font-black uppercase tracking-[0.2em] opacity-80">Category Spotlight</p>
            ...
            <div className="flex w-full flex-col gap-3 md:w-auto md:flex-row">
              <div className="relative min-w-[280px]">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-black/10 bg-white py-2 pl-9 pr-3 outline-none"
                />
              </div>
            </div>
          </div>
        ...
        <div className="mb-6 inline-flex items-center gap-3 rounded-2xl border-2 border-black bg-[#090909] px-4 py-3 text-white shadow-[0_6px_0_0_rgba(0,0,0,0.9)]">
          <Zap className="h-4 w-4 text-[#ffb400]" />
          <p className="text-sm font-black uppercase tracking-[0.18em]">Browse the latest Maple Deals picks</p>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16 md:px-10 lg:px-12">
        <div className="rounded-[2rem] border-2 border-black bg-[linear-gradient(135deg,#090909_0%,#1f1f1f_58%,#b91c1c_100%)] px-8 py-10 text-white shadow-[0_14px_0_0_rgba(0,0,0,0.95)]">
          <div className="grid gap-8 lg:grid-cols-[1.3fr_0.9fr] lg:items-center">
            <div>
              <p className="mb-3 text-sm font-black uppercase tracking-[0.28em] text-[#ffb400]">Ready to grow</p>
              ...
              <p className="mt-4 max-w-2xl text-neutral-300">
                This version leans much harder into your logo style with bold outlines, stacked promo blocks, stronger contrast, and a more branded deal look across the homepage.
              </p>
            </div>
